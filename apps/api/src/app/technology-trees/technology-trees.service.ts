...
export class TechnologyTreesService {
    ...
    public async update(data: TechnologyTree) {
        ...
        if (!byId) {
            throw new NotFoundException(
                `Could not update Technology Tree with ID "${data.id}". - Reason: Could not find Technology Tree`
            );
        }
        if (await this.isNameTaken(data.name, data.id)) {
            throw new BadRequestException(
                `Could not update Technology Tree with ID "${data.id}". - Reason: Name "${data.name}" is already in use`
            );
        }
        return await this.technologyTreesRepository.update(data);
    }
    ...
    private async isNameTaken(name: string, technologyTreeId?: string) {
        const byName = await this.getByName(name);
        return Boolean(byName) && (!technologyTreeId || technologyTreeId !== byName.id);
    }
}
