...
`tls internal` generates its local CA and issues a leaf cert on first use, which can take a moment — a CI runner is typically slower or colder-started than a local machine that's already reusing cached Caddy state, so the very first handshake can land before the cert is ready and fail with `SSL_ERROR_SYSCALL`. `curl`'s `--retry` (even combined with `--retry-connrefused`) only covers a fixed set of transient conditions — timeouts, FTP 4xx, HTTP 408/429/500/502/503/504 — and does not retry SSL/TLS handshake failures; `--retry-all-errors` is needed for that, confirmed against curl's own documentation rather than assumed from the flag names.

A new `e2e-web` job (`needs: docker-web`) pulls the just-pushed `pr-<N>` tag from GHCR rather than reusing a local build from the same CI run — this tests the literal artifact that would later be promoted to `next` ([ADR-0037](./0037-promote-web-image-via-retag-not-rebuild.md)'s build-once principle), and a multi-platform `buildx bake --push` result isn't loadable into the local Docker daemon in a runnable form anyway. `docker-web` (build+push, renamed from the previous `docker` job) and `e2e-web` (pull, compose up, smoke-test) stay as two separate jobs rather than one combined job: separate check runs distinguish "the image failed to build" from "the app/E2E environment is broken" in the PR checks list, and a transient E2E flake can be re-run on its own via GitHub's "re-run failed jobs" without rebuilding and re-pushing the image.

There's no explicit `docker compose down` teardown step: `runs-on: ubuntu-24.04` is a GitHub-hosted runner, and those are fully ephemeral — each job gets a fresh VM destroyed immediately after the job ends, regardless of outcome, taking any containers/networks `compose up` created with it. An explicit teardown would only matter on a persistent or self-hosted runner, which this isn't.
