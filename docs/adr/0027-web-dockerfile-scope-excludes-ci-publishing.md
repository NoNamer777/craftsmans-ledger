...
Issue #92 asked only to containerize `web`; no deployment target or container registry has been chosen anywhere in this repo yet. `apps/web/.docker/Dockerfile` and `apps/web/.docker/default.conf` are added for local/manual builds (`docker build -f apps/web/.docker/Dockerfile -t craftsmans-ledger-web .`) only — neither `.github/workflows/pull-request.yml` nor `push-main.yml` build or push the image. Building it in CI now, with nothing to deploy it to yet, it would be scope creep the issue didn't ask for and premature given the registry/target is still undecided.
