...
- A future app's image belongs at `ghcr.io/nonamer777/craftsmans-ledger/<app-name>`, not a new flat `craftsmans-ledger-<app-name>` package.
- The local-build tag documented in `.docker/web/Dockerfile`'s header comment (`craftsmans-ledger-web`) intentionally stays a separate, flatter name — it's a throwaway local tag, not the published artifact's identity, so the two naming schemes are allowed to diverge.
